...
def addFillerBits(code):
    ...
    for bit in code:
        if bit == '1':
            bits1 = (bits1 + 1, 0)[bits1 == 5] # Si ya hay cinco bits 1 seguido, se reinicia el contador sino se auments en uno 
            newCode = (newCode + bit, newCode + bit + '0')[bits1 == 5] #si hay 5 bits 1 seguido se colocal el bit actual y se le agrega el relleno sino solo se copia el bit actual 
        else:
...
def removeBandBits(code):
    ...
    if BAND == code[0:8] and BAND == code[length-8:length]:
        return (True, code[8:len(code)-8]) #retorna el codigo si bandera
    else:
        return (False, 'Codigo invalido, no contiene los bits de bandera') #sino se retorna  un error
